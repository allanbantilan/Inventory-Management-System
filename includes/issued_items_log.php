...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <div class="row">
                <div class="col">
                    <h6 class="m-2 font-weight-bold text-primary">Issued Items Log</h6>
                </div>
                <!-- <div class="col">
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="dataTableHis" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT * FROM issued_items_log";
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td>" . $row['items_id_log'] . "</td>";
                                echo "<td>" . $row['item_serial_number_log'] . "</td>";
                                echo "<td>" . $row['item_type_log'] . "</td>";
                                echo "<td>" . $row['item_status_log'] . "</td>";
                                echo "<td>" . $row['item_description_log'] . "</td>";
                                echo "<td>" . $row['item_dateissued_log'] . "</td>";
                                echo "<td>" . $row['item_issued_in_out'] . "</td>";
                                echo "<td>" . $row['item_site_log'] . "</td>";
                                echo "<td>" . $row['item_issued_to_log'] . "</td>";
                                echo "<td>" . $row['item_issued_by'] . "</td>";
...
<script>
    $(document).ready(function () {
        ...
        var dataTable = $('#dataTableHis').DataTable({
            "order": [[0, 'DESC']] // Sort the first column (index 0) in ascending order
        });
        ...
        $('#category').on('change', function () {
            ...
            if (category === 'all') {
                dataTable.column(2).search('').draw(); // Clear the search
            } else {
        ...
        $('#siteDrop').on('change', function () {
            var site = $(this).val();
            if (site === 'all') {
                dataTable.column(6).search('').draw(); // Clear the search
            } else {
                dataTable.column(6).search(site).draw();
            }
        });
        ...
        $('#status').on('change', function () {
            var status = $(this).val();
            if (status === 'all') {
                dataTable.column(3).search('').draw(); // Clear the search
            } else {
