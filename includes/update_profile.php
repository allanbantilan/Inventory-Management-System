...
if ($result->num_rows > 0) {
    ...
    if ($updateQuery->execute()) {
        ...
        echo "<script>alert('Profile updated successfully.'); window.location.href = '../dashboard1.php';</script>";
        exit;
    } else {
        // Error updating profile
        echo "<script>alert('Error updating profile. Please try again later.'); window.location.href = '../dashboard1.php';</script>";
        exit;
    }
} else {
    // Current password is incorrect
    echo "<script>alert('Current password does not match.'); window.location.href = '../dashboard1.php';</script>";
    exit;
}
